...
0.5(\tilde{B} + \tilde{B}^T)$ which yields a symmetric matrix. We fix $\tau =
1$ and for each $\rho$ we generate 10000 datasets of 100 individuals from CAR
model. \autoref{tab:correlations-car-model} shows the 12.5\% and 87.5\%
quartiles of the Pearson correlations among the individuals. Notice that the
intervals are not linear and even with high values of $\rho$, the correlation
is not much larger. 

\begin{table}[hb]
  \centering
  \caption{\label{tab:correlations-car-model}Interval of correlations among
  individuals for different values of $\rho$}
  \begin{tabular}{ccc}
  \hline
  \multirow{2}{*}{$\rho$} & \multicolumn{2}{c}{Interval 75\%} \\ \cline{2-3} 
   & Quartile 12.5\% & Quartile 87.5\% \\ \hline
  0.5 & -0.005 & 0.028 \\
  0.8 & 0.017 & 0.06 \\
  0.9 & 0.055 & 0.113 \\
  0.99 & 0.426 & 0.526 \\
  0.999 & 0.884 & 0.918 \\ \hline
  \end{tabular}
