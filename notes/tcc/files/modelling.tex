...
simulated probabilities of having the disease $\theta_i$. The green area is delimited by the curves generated
by $2\sqrt{\theta_i(1-\theta_i)/n}$, where $n=500$ is the number of points. 
This area is a proxy for $\pm 2$ standard-error bounds. Although we are performing Bayesian inference, frequentist properties can me
...
\begin{figure}[ht]
  ...
  \eqref{model:imperfect-tests-rds}}
  \includegraphics[width=14cm]{trace_plot_experiment4_imperfect_test.pdf}
  \fonte{Prepared by the author (2021) from the Stan sampling result.}
\end{figure}
...
some problems that can arise. Specifying the hyperparameters with $\alpha_p =
...
strong prior for $\gamma_s$ can handle this effect. A similar behavior happens
...
as $\sum_{i=1}^n \omega_i = 0$. This is commonly added in Stan code with 
$$
