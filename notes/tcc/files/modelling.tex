...
\begin{alineas}
  \item {\em Homophily model}: Ignoring homophily effects can derive biased
  ...
  $\gamma$ to measure homophily effects. In our model it would change the
  formulation of $\theta_i$ to 
  $$
  g(\theta_i) = g(\theta) + \boldsymbol{X} \beta + \gamma\frac{1}{n_i}\sum_{k \sim j}z_{k} + \omega_i,
  $$
  where $n_i$ is the number of connections of an individual and $z$ is a
  variable that influences the recruitment choice. The parameter $\gamma$ as
  ...
  \item {\em Sampling weights}: one of the objectives of the introduction of 
  Volz-Heckathorn estimator for prevalence was to consider the inclusion
  probabilities of the individuals in the estimator. Our model does not
  consider these probabilities, which can lead to wrong inferences.
  \textcite{bastos2018hiv} uses the pseudo-likelihood strategy to powder the
  likelihood of each individual. When point estimates are considered, the
  definition of a loss function which makes this correction can also be a
  valid solution. For instance, if 
  \item {\em Bottlenecks}
\end{alineas}