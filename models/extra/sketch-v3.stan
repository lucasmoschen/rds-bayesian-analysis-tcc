  ...
  */
  real sparse_car_lpdf(vector phi, vector mu, real tau, real alpha, 
    int[,] W_sparse, vector D_sparse, vector lambda, int n, int W_n) {
      ...
      phit_D = (phi_center .* D_sparse)';
      phit_W = rep_row_vector(0, n);
      for (i in 1:W_n) {
        phit_W[W_sparse[i, 1]] = phit_W[W_sparse[i, 1]] + phi_center[W_sparse[i, 2]];
        phit_W[W_sparse[i, 2]] = phit_W[W_sparse[i, 2]] + phi_center[W_sparse[i, 1]];
      }
    
      for (i in 1:n) ldet_terms[i] = log1m(alpha * lambda[i]);
      return 0.5 * (n * log(tau) + sum(ldet_terms) - tau * (phit_D * phi_center - alpha * (phit_W * phi_center)));
  }
...
model {
  ...
  phi ~ sparse_car(mu, tau, alpha, W_sparse, D_sparse, lambda, n, W_n);
