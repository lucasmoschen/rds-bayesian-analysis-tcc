...
$$
For now, it is hard to verify if $\alpha_3 > 0$ only when $\sqrt{\Delta}$ is
summed. Therefore, we need to verify numerically. With these three
expressions, from $\alpha_1, \alpha_2$, and $\alpha_3$, the last equation from
the system \eqref{eq:system-moments-alpha} indicates the solution $\alpha_4$.
Numerically, we reduced the dimension of the problem. 
